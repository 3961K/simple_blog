...
- simple blogは基本的なWebアプリケーションの学習としてDjangoを用いて作成した記事投稿機能やユーザフォロー機能などを持つ簡易的なブログサービスです。
...
- (※) 試して頂く場合に登録工程を簡潔にするため、入力されたメールアドレスに対して確認用のメールを送信する事等は行わないので、試して頂く場合はご利用しているメールアドレスではなく、メールアドレスの形式の文字列を入力して頂くだけで問題無いです。
